...
/* Largest scale, stepping down from `start`, at which `text` fits `maxWidth`.

   Every screen in this game is laid out in the composite buffer's own pixels,
   and that buffer is whatever size the player's window is. A string drawn at
   a fixed scale is therefore fine on the monitor it was written on and runs
   off the edge on a narrower one - which is not a phone problem, it was
   overflowing the prompt band by 91px at 1280x720.

   Stops at `min` (1 by default, never below): this is a bitmap font, so
   scaling below 1 resamples the glyph grid and destroys the letterforms.
   When even `min` does not fit, the caller needs to wrap or shorten instead,
   and can detect that by measuring the returned scale. */
function fitScale(text, maxWidth, start, tracking, min) {
  if (!state.ready) init();
  const floor = min === undefined ? 1 : min;
  let s = Math.max(floor, start);
  while (s > floor && measure(text, s, tracking) > maxWidth) s -= 0.5;
  return s;
}

/* Fit if possible, wrap if not, and never exceed `maxLines`. Returns the
   lines to draw plus the scale they were laid out at. */
function fitLines(text, maxWidth, start, tracking, maxLines, min) {
  const scale = fitScale(text, maxWidth, start, tracking, min);
  if (measure(text, scale, tracking) <= maxWidth) return { lines: [text], scale };
  const lines = wrap(text, maxWidth, scale, tracking);
  return { lines: lines.slice(0, Math.max(1, maxLines || lines.length)), scale };
}
