...
/*   drill     one question type, endless, and a wrong answer costs you the
               shot and the streak but not the run - the point is repetition on
               a weakness, and dying every time you get one wrong would make
               that impossible.                                            */
const KIND = { INFINITY: 'infinity', MODULE: 'module', FULL: 'full', DRILL: 'drill' };
  ...
  items: [
    { key: 'play', label: 'PLAY', items: [
      { key: 'english', label: 'ENGLISH', note: 'READING AND WRITING QUESTIONS ONLY', items: [
        ...
        { key: 'rw-types', label: 'SPECIFIC QUESTION TYPES',
          note: 'PICK ONE TYPE AND DRILL IT   WRONG ANSWERS DO NOT END THE RUN',
          action: 'types', section: SECTION.RW }
      ] },
      { key: 'math', label: 'MATH', note: 'MATH QUESTIONS ONLY', items: [
        ...
        { key: 'math-types', label: 'SPECIFIC QUESTION TYPES',
          note: 'PICK ONE TYPE AND DRILL IT   WRONG ANSWERS DO NOT END THE RUN',
          action: 'types', section: SECTION.MATH }
      ] },
...
function modeLabel(mode) {
  if (!mode) return '';
  /* A mode may name itself, which a single-question-type drill launched from
     the logbook does: "MATH INFINITY" would be true but useless there, since
     the whole point of the run is which type it narrowed to. */
