...
const { describeResponse } = SATG.satUtil;
...
class ExamForm {
  ...
  grade(bank) {
    ...
    /* Question type is finer than skill and is what the analysis report is
       keyed on: "Percentages" is one skill and four different questions, and a
       student is regularly fine at some and lost at others. */
    const perQType = {};
    const qtypeSection = {};

    /* Accuracy by difficulty. Every question has carried an easy/medium/hard
       tag since the generators were written and nothing has ever read it, which
       is a waste: "missing the hard ones" and "careless on the easy ones" are
       different problems with different fixes, and the report cannot tell them
       apart without this. */
    const perDifficulty = {};

    /* One record per question, in the order the player saw them.

       This is what the two review features need and what grading previously
       threw away: the per-question right/wrong list, and TEXT ANALYSIS, which
       reopens the paper so the player can see the answer they actually gave.
       The module already keeps questions and responses in parallel arrays, so
       nothing new has to be captured during the exam - it only has to survive
       grading. */
    const items = [];
    ...
    for (const mod of this.modules) {
      ...
      for (let i = 0; i < mod.questions.length; i++) {
        ...
        bump(perQType, q.qtype, right);
        bump(perDifficulty, q.difficulty, right);
        skillSection[q.skill] = q.section;
        if (q.qtype) qtypeSection[q.qtype] = q.section;

        items.push({
          n: items.length + 1,          // 1-based position across the whole form
          module: mod.number,
          section: mod.section,
          indexInModule: i,
          domain: q.domain,
          ...
          answered, right,
          /* The response as given, plus what the key was, so the review can
             show both without re-deriving either. For multiple choice the
             response is a choice index, so resolve it to the letter and text
             the player actually clicked. */
          response: answered ? r : null,
          responseText: answered ? describeResponse(q, r) : null,
          correctText: q.answerText != null ? String(q.answerText) : null,
          /* Carried so a review can explain the question without the form
             still being alive - which it will not be, once the run is saved
             and reopened from STATS days later. */
          explanation: q.explanation || null,
          /* Why the option the player actually chose is wrong - the one line a
             review can give that a generic explanation cannot. Null when they
    ...
    return {
      ...
      perQType: Object.keys(perQType).map((k) => ({
        qtype: k, section: qtypeSection[k] || null,
        right: perQType[k].right, total: perQType[k].total,
        pct: perQType[k].total ? perQType[k].right / perQType[k].total : 0
      })).sort((a, b) => b.pct - a.pct || b.total - a.total),
      perDifficulty: ['easy', 'medium', 'hard']
        .filter((d) => perDifficulty[d])
        .map((d) => ({
          difficulty: d,
          right: perDifficulty[d].right, total: perDifficulty[d].total,
