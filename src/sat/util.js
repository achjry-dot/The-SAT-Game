...
function buildDistractors(correct, candidates, count, fallback) {
  ...
  if (out.length < count) {
    console.warn('[sat] only ' + out.length + ' distractors for key "' + correct + '"');
  }
...
function makeMC(rng, spec) {
  const distractors = buildDistractors(spec.correct, spec.distractors, 3, spec.fallback);
  const { choices, answerIndex } = packChoices(rng, spec.correct, distractors);
  return {
    id: ++questionSerial,
    ...
    stem: spec.stem,
    format: 'mc',
    choices: choices.map((c, i) => ({ letter: LETTERS[i], text: String(c) })),
    answerIndex,
    answerText: String(choices[answerIndex]),
...
SATG.satUtil = {
  ...
  packChoices, buildDistractors,
  makeMC, makeGrid, scoreDifficulty, LETTERS
};
