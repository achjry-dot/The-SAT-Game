/* =========================================================================
   ...
   States
     ...
     lose      pitch black, YOU LOOSE, RETRY / QUIT
...
class Game {
  ...
  activateTitle() {
    switch (this.title.selected) {
      ...
      case 'continue': if (this.save) this.startRun(true); break;
  ...
  renderExitScreen(P) {
    ...
    F.draw(ctx, 'SESSION TERMINATED', this.lose.W / 2, this.lose.H / 2 - 40 * s,
           { color: '#8e8779', scale: 3 * s, tracking: 4 * s, align: 'center' });
    F.draw(ctx, 'CLICK ANYWHERE TO RETURN', this.lose.W / 2, this.lose.H / 2 + 30 * s,
           { color: '#4f4a42', scale: s, tracking: 2 * s, align: 'center' });
    this.lose.upload();
    P.drawOverlay(this.lose.texture, { x: 0, y: 0, w: 1, h: 1 });
  }
}
